            ...
            <div class="d-flex align-items-center">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb m-0 p-0">
                        ...
                        <li class="breadcrumb-item"><a>Agusta Properti</a>
                        </li>
                    </ol>
                </nav>
            </div>
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Agusta Properti</h4>
                <h6 class="card-subtitle">Berdasarkan produk Agusta Properti yang paling banyak dipesan saat membuat janji temu.</h6>
...
<script>
    ...
    function getData(awal, akhir) {
        $.ajax({
            type: 'POST',
            url: '{{ route("admin.report.product.prop") }}',
            data: {
                "_token": "{{ csrf_token() }}",
                "awal": awal,
                "akhir": akhir
            ...
            success: function(data) {
                ...
                if (data.products.length > 0) {
                    $.each(data.products, function(index, product) {
                        var categoryText = ((product.category == 'House') ? 'Rumah' : ((product.category == 'Land') ? 'Tanah' : product.category));
                        var typeText = ((product.type == 'Sell') ? 'Jual' : ((product.type == 'Rent') ? 'Sewa' : 'Jual & Sewa'));

                        var row = '<tr><td>' + (index + 1) +
                            '</td><td>' + product.id +
                            '</td><td>' + categoryText +
                            '</td><td>' + typeText +
                            '</td><td>' + product.regency +
                            '</td><td>' + product.title +
